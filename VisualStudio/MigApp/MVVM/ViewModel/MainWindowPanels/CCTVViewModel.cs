...
{
    internal class CCTVViewModel : Core.ViewModel
    {
        ...
        public async Task LoadTableAsync()
        {
            try
            {
                DataTable temp = await pgsql.GetTable("id, inventory_number, model, resolution, ip, street, microphone, location, comment", "\"Technic\".cctv", Filter);
                ...
                temp.Columns["street"].ColumnName = "Уличная";
                temp.Columns["microphone"].ColumnName = "Микрофон";
