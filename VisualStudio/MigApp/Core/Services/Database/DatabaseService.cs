...
namespace MigApp.Core.Services
{
    public class DatabaseService : IDatabaseService
    {
        ...
        public DatabaseService(IAppLogger logger, IUINotificationService ui, IDbContextProvider provider, ISecurityService securityService)
        {
            _logger = logger;
            _ui = ui;
            _provider = provider;
        ...
        private async Task<bool> CheckConnectionStep(MigDatabaseContext context, string stepName, Func<Task<bool>> checkAction)
        {
            ...
            {
                LogAndShowError(pgEx, $"Ошибка PostgreSQL при проверке {stepName} (код: {pgEx.SqlState})");
                return false;
            }
            catch (Exception ex)
            {
                LogAndShowError(ex, $"Ошибка при проверке {stepName}");
                return false;
            }
        }

        private async void LogAndShowError(Exception ex, string message)
        {
